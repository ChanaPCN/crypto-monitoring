...
export default function SignUpPage() {
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        try {
            // Check rate limit using database-backed API
            const rateLimitResponse = await fetch('/api/rate-limit', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    identifier: `signup:${fingerprint}`,
                    maxAttempts: 3,
                    windowMs: 60 * 60 * 1000 // 1 hour
                })
            })

            const rateLimitCheck = await rateLimitResponse.json()
                ...
                <div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-sm border border-gray-200/60 p-6 sm:p-8">
                    ...
                    <form className="space-y-5" onSubmit={handleSignUp}>
                        <div className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="confirm-password" className="block text-sm font-semibold text-gray-700 mb-2 tracking-tight">
                                    Confirm Password
                                </label>
                                <input
                                    id="confirm-password"
